'''
피로도 시스템(0 이상의 정수로 표현합니다)이 있으며, 일정 피로도를 사용해서 던전을 탐험할 수 있습니다. 
...
1. 파이썬의 itertools를 이용해 모든 조합을 생성
2. 각 조합마다 result를 계산
3. 가장 큰 result을 return
'''
import itertools
...
def solution(k, dungeons):
    ...
    dungeon_permutation = itertools.permutations(dungeons, dungeons_len)
    
    for dungeon in dungeon_permutation:
        k_copy = k
        result = 0
        for d in dungeon:
            if k_copy >= d[0]:
